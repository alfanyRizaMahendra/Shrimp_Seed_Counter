...
def inputCam(source):
    ...
    if not cam.isOpened():
        ...
        return 0
    self.capWebcam.set(cv2.CAP_PROP_FRAME_WIDTH, 1920)
    self.capWebcam.set(cv2.CAP_PROP_FRAME_HEIGHT, 1080)
    ...
    for threshold in threshold_setup:
        ...
        for folder in test_images_folders:
            ...
            for model in models:
                ...
                np.append(detected_total, int(detection_result))
    
    return np.amax(detected_total)
...
class buttonImg:
    def __init__(self, obj, imgDir, size, position, command=None):
        self.obj= obj
        self.imgDir= imgDir
        self.size= size
        self.position= position
    ...
    def buttonShow(self):
        self.Button_ = Button(self.obj, width = self.size[0], height = self.size[1], fg = '#000', bg = '#000', bd = 10, highlightthickness=4, highlightcolor="#000", highlightbackground="#000", borderwidth = 4, command=self.command)         
        self.Button_.place(x=self.position[0],y=self.position[1])
...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.sky = logo(self, 'icon/penssky.png', [215, 55], [800,0], bg = '#d4e4e8')
        self.sky.show()
        ...
        self.button3 = buttonImg(self, 'icon/exit.png', [60,60], [920,685], lambda : self.close())
...
class Page1(tk.Frame):
    def __init__(self, parent, controller):
        ...
        fontStyleLabel = tkFont.Font(family = "Arial", size = 21)
        # ---------------------- comboBox -------------------------
        ...
        # ----------------------- slider ---------------------------
        redLabel = sliderLabel(self, labelText="Merah", bgColor='#444', labelPosition = [80,50], labelFont = "Arial", labelFontSize = 21)
        greenLabel = sliderLabel(self, labelText="Hijau", bgColor='#444', labelPosition = [290,50], labelFont = "Arial", labelFontSize = 21)
        blueLabel = sliderLabel(self, labelText="Biru", bgColor='#444', labelPosition = [520,50], labelFont = "Arial", labelFontSize = 21)
        
        self.redNow = tk.DoubleVar()
        self.greenNow = tk.DoubleVar()
        self.blueNow = tk.DoubleVar()
        
        self.red = ttk.Scale(self, from_=0, length = 200, to=255, orient='horizontal', command=self.getRedColor, variable=self.redNow)
        self.red.place(x=10,y=90)
        self.green = ttk.Scale(self, from_=0, length = 200, to=255, orient='horizontal', command=self.getGreenColor, variable=self.greenNow)
        self.green.place(x=230,y=90)
        self.blue = ttk.Scale(self, from_=0, length = 200, to=255, orient='horizontal', command=self.getBlueColor, variable=self.blueNow)
        self.blue.place(x=450,y=90)
        
        self.redValue = Label(self, text="0", bg='#444', fg='#fff', font=fontStyleLabel)
        self.redValue.place(x=80,y=120)
        
        self.greenValue = Label(self, text="0", bg='#444', fg='#fff', font=fontStyleLabel)
        self.greenValue.place(x=290,y=120)
        
        self.blueValue = Label(self, text="0", bg='#444', fg='#fff', font=fontStyleLabel)
        self.blueValue.place(x=520,y=120)
        
        label1 = Label(self, text="Pastikan Wadah Benih\nUdang Terlihat Jelas\nMelalui Kamera", bg='#444', fg='#fff', font=fontStyleLabel)
        label1.pack()
        label1.place(x=710,y=50)

        self.back = buttonImg(self,'icon/home.png', [130,130], [780,600], lambda : [controller.show_frame(StartPage), videoStream.onClose(self.videoObj)])
        self.back.buttonShow()

        self.button2 = buttonL(self,[16,2],[710,280],"Camera On",fontStyle,18,[BlackSolid,_from_rgb((244,239,140))],lambda : [self.cameraState()])
        self.button2.buttonShow()
        
        button3 = buttonL(self,[16,2],[710,440],"Record",fontStyle,18,[BlackSolid,_from_rgb((255,190,100))],lambda : [videoStream.onStart(self.videoObj, bgColor = self.backColor, recordTime = 2, record = "yes")])
        button3.buttonShow()
...
class videoStream(tk.Frame):
    ...
    def onStart(self, bgColor = '255,255,255', recordTime = 1, record = "no", cameraFlag = False):
        self.record = record
        self.recordTime = recordTime * 60 * 1000 # minutes from ms
        self.outVideo = "record/" + bgColor + " - " + self.timeString + ".mp4"
